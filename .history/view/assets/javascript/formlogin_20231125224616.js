...
document.body.addEventListener("click", (event) => {
  if (
    event.target.matches(".closeLogin") ||
    event.target.matches(".body-form-login")
  ) {
    const bodyLogin = document.querySelector(".body-form-login");
    const modalLogin = document.querySelector(".modal-login");

    setTimeout(() => {
      modalLogin.style.transform = "scale(0.1)";
      modalLogin.style.opacity = "0";
      modalLogin.style.visibility = "hidden";
      bodyLogin.parentNode.removeChild(bodyLogin);
    });
  }
});
